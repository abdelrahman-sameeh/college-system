    ...
    StudentModel
        ...
        .then(Students => {
            ...
            for (let i = 0; i < Students.length; i++) {
                obj.push({No:i+1,Accademic_Num:Students[i].accademic,studentName:Students[i].name})
                for (let j = 1; j <=12; j++) {
                    obj[i][`Week${j}`]=" "
                }
            }
            ...
            fs.cp(path.join(myPath, `${sub}.xlsx`) ,path.join(myPath, 'assets', 'adminAbsenceFiles', `${sub}.xlsx`), (err)=>{
...
exports.getStudentName = (req, res, next) => {
    ...
    StudentModel
        .getItemByName(req.session.name)
        .then(Student => {
