...
exports.getHome = ((req, res, next) => {
    res.render("index", {
        ...
        successDownloadAbsenceFileByAdmin: req.flash('successDownloadAbsenceFileByAdmin')[0],
        failedDownloadAbsenceFileByAdmin: req.flash('failedDownloadAbsenceFileByAdmin')[0]
    })
})
...
exports.postSignup = ((req, res, next) => {
    if (validationResult(req).isEmpty()) {
        user.addToSignup(req.body.username, req.body.type, req.body.password, req.body.accademic, req.body.prevSub)
...
exports.postlogin = ((req, res, next) => {
    if (validationResult(req).isEmpty()) {
        user.addToLogin(req.body.username, req.body.password)
            .then((user) => {
...
exports.postChangePassword = (req, res, next) => {
    ...
    if (validationResult(req).isEmpty()) {
        user.changePassword(name, userId, newPassword, oldPassword)
            .then((msg) => {
                res.redirect('/')
...
exports.postChangeImage = (req, res, next) => {
    ...
    if (validationResult(req).isEmpty()) {
        ...
        user.changeImage(name, userId, image)
            .then(() => {
                res.redirect('/');
...
exports.getEditPageUser = (req, res, next) => {
    ...
    res.render('AuthPages/editUser', {
        ...
        validationErr: req.flash('validationErr')[0],
        userImage: req.query.userImage || 'user-image.jpg',
        userType: req.query.type,
        userAcademic: req.query.userAcademic,
...
exports.postEditPage = (req, res, next) => {
    ...
    const { userAcademic } = req.body;

    if (validationResult(req).isEmpty()) {
        user.changePasswordByAdmin(username, _id, password)
            .then(() => {
                res.redirect('/users');
            ...
            .catch(err => {
                console.log(err);
                res.redirect(`/users/edit/${_id}?username=${username}&userImage=${userImage}&userAcademic=${userAcademic}`)
...
exports.postDeleteImageByAdmin = (req, res, next) => {
    ...
    if (userImage == 'user-image.jpg') {
        ...
        res.redirect(`/users/edit/${_id}?username=${username}&userImage=${defaultImage}&userAcademic=${userAcademic}`)
        return
    } else {
        user.changeImage(username, _id, defaultImage)
            .then(() => {
                res.redirect(`/users/edit/${_id}?username=${username}&userImage=${defaultImage}&userAcademic=${userAcademic}`)
