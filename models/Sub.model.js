        ...
        mongoose
            ...
            .then(items => {
                if (!items) {
                    ...
                    return SubjectItem.findByIdAndUpdate(items._id, data)
                }
            })
            .then(() => {
                mongoose.disconnect();
...
exports.deleteItem = (id) => {
    return new Promise((resolve, reject) => {
        mongoose
            .connect(DB_URL)
            .then(() => SubjectItem.findOneAndDelete({ subName: id }))
            .then(() => {
                mongoose.disconnect();
                resolve();
            })
        ...
        mongoose
            ...
            .then(sub => {
                if (sub.material.includes(filename)) {
                    ...
                    return SubjectItem.findOneAndUpdate({ subName, subDoc }, { $push: { material: filename } })
                }
            })
            .then(() => {
                mongoose.disconnect();
...
exports.deletePdfByName = (data) => {
    ...
    return new Promise((resolve, reject) => {
        mongoose
            .connect(DB_URL)
            .then(() => {
                return SubjectItem.findOneAndUpdate({ subName, subDoc }, { $pull: { material: filename } })
            })
            .then(() => {
                mongoose.disconnect();
