...
exports.addToSignup = (username, type, password, accademic, prevSub) => {
    ...
    return new Promise((resolve, reject) => {
        mongoose
            ...
            .then(() => {
                accademic = accademic || 0;
                prevSub = prevSub || "empty"
                let user = new User({
                    username: username,
                    password: password,
                    accademic: accademic,
        ...
        mongoose
            ...
            .then((user) => {
                if (user.password == oldPassword) {
                    return User.findOneAndUpdate({ _id: userId, username }, { password: newPassword })
                } else {
                    mongoose.disconnect()
...
exports.changePasswordByAdmin = (username, userId, newPassword) => {
    return new Promise((resolve, reject) => {
        mongoose
            .connect(db_url)
            .then(() => {
