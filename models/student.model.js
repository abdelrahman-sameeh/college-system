        ...
        mongoose
            ...
            .then((student) => {
                ...
                if (Object.keys(student.absence[subName]).length) {
                    return Students.findOneAndUpdate({ name, accademic }, { absence: { ...student.absence, [subName]: newDate } })

                } else {
                    return Students.findOneAndUpdate({ name, accademic }, { absence: { ...student.absence, [subName]: data } })
                }
            })
            .then(() => {
                mongoose.disconnect();
...
const setGrade = (name, accademic, grade, subName) => {
    ...
    return new Promise((resolve, reject) => {
        mongoose
            ...
            .then((student) => {
                return Students.findOneAndUpdate({ name, accademic }, { grades: { ...student.grades, [subName]: { grade: +grade, status, rate } } })
            })
            .then(() => {
                mongoose.disconnect();
                resolve()
